...
public class Chunk : MonoBehaviour
{
    ...
    public static Chunk GetChunk(Vector3 wPos)
    {
        for (int i = 0; i < chunks.Count; i++)
        {
            ...
            if ((wPos.x < tempPos.x) || (wPos.z < tempPos.z) || (wPos.x >= tempPos.x + width) || (wPos.z >= tempPos.z + width))
                continue;
    ...
    int GenerateHeight(Vector3 wPos)
    {
        ...
        int h = Mathf.FloorToInt(noise0 + noise1 + noise2 + baseHeight);
        if(amplitude!= 0 && h <= -2) h = h * -1;
    ...
    public void BuildBlock_v2(BlockType typeid, int x, int y, int z)
    {
        if((x < 0) || (y < 0) || (z < 0) || (x >= width) || (y >= height || (z >= width))) {
            Vector3 p = new Vector3(x,y,z) + transform.position;
            Chunk chunk = null;
            if(x < 0)
                chunk = GetChunk(new Vector3(x - 2, y, z) + transform.position);
            if(z < 0)
                chunk = GetChunk(new Vector3(x, y, z - 2) + transform.position);
            if(x >= width)
                chunk = GetChunk(new Vector3(x + 2, y, z) + transform.position);
            if(z >= width)
                chunk = GetChunk(new Vector3(x, y, z + 2) + transform.position);
            ...
            if(chunk != null){
                p -= chunk.transform.position;
                chunk.BuildBlock_v2(typeid, Mathf.FloorToInt(p[0]), Mathf.FloorToInt(p[1]), Mathf.FloorToInt(p[2]));
            }
            return;
		}
        if (map[x, y, z] == typeid) return;
        map[x, y, z] = typeid;
